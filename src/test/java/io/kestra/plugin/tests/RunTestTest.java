...
public class RunTestTest extends AbstractKestraEeContainerTest {
    ...
    @Test
    public void crashingTest_shouldMarkTask_asFailed() throws Exception {
        // given
        var runContext = runContextFactory.of();
        var flow = kestraTestDataUtils.createRandomizedFlow(NAMESPACE);
        ...
        var runTestTask = RunTest.builder()
            ...
            .tenantId(Property.ofValue(TENANT_ID))
            .namespace(Property.ofValue(NAMESPACE))
            .testId(Property.ofValue(crashingTestSuite.getId()))
            .build();
        var runTestOutput = runTestTask.run(runContext);

        // then
        assertThat(runTestOutput).extracting(RunTest.Output::getResult).extracting(TestSuiteRunResult::getTestSuiteId).isEqualTo(crashingTestSuite.getId());
        assertThat(runTestOutput).extracting(RunTest.Output::getResult).extracting(TestSuiteRunResult::getState).isEqualTo(TestState.ERROR);
        assertThat(runTestOutput.getTaskStateOverride()).isEqualTo(Optional.of(State.Type.FAILED));
    }

    @Test
    public void failingTest_shouldMarkTask_asWarningByDefault() throws Exception {
        // given
        var runContext = runContextFactory.of();
        var flow = kestraTestDataUtils.createRandomizedFlow(NAMESPACE);
        ...
        var runTestTask = RunTest.builder()
            ...
            .tenantId(Property.ofValue(TENANT_ID))
            .namespace(Property.ofValue(NAMESPACE))
            .testId(Property.ofValue(failingTestSuite.getId()))
            .build();
        var runTestOutput = runTestTask.run(runContext);

        // then
        assertThat(runTestOutput).extracting(RunTest.Output::getResult).extracting(TestSuiteRunResult::getTestSuiteId).isEqualTo(failingTestSuite.getId());
        assertThat(runTestOutput).extracting(RunTest.Output::getResult).extracting(TestSuiteRunResult::getState).isEqualTo(TestState.FAILED);
        assertThat(runTestOutput.getTaskStateOverride()).isEqualTo(Optional.of(State.Type.WARNING));
    }

    @Test
    public void failingTest_shouldMarkTask_asWFailedWhenRequested() throws Exception {
        // given
        var runContext = runContextFactory.of();
        var flow = kestraTestDataUtils.createRandomizedFlow(NAMESPACE);
        var failingTestSuite = kestraTestDataUtils.createRandomizedFailingTestSuite(NAMESPACE, flow.getId());
