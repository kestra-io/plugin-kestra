...
public class ScheduleMonitor extends AbstractKestraTrigger implements TriggerOutput<ScheduleMonitor.Output>, PollingTriggerInterface {
    ...
    public Output runChecks(RunContext runContext) throws Exception {
        ...
        while ((long) (page - 1) * size < total) {
            PagedResultsApiTriggerAndState response = client.triggers().searchTriggers(tenantId, page, size, null, filters, null);
            ...
            for (ApiTriggerAndState t : results) {
                ...
                ApiTriggerState triggerContext = t.getState();

                if (triggerContext == null)
                    continue;

                boolean isDisabled = Boolean.TRUE.equals(t.getTrigger().getDisabled()) || Boolean.TRUE.equals(triggerContext.getDisabled());

                Instant now = Instant.now();
                Instant lastExec = triggerContext.getLastTriggeredDate() != null ? triggerContext.getLastTriggeredDate().toInstant() : null;
                Instant nextExec = triggerContext.getNextEvaluationDate() != null ? triggerContext.getNextEvaluationDate().toInstant() : null;
                ...
                if (rMaxExecutionDuration != null && triggerContext.getExecutionId() != null) {
                    ApiExecution exec = client.executions().execution(triggerContext.getExecutionId(), tenantId);
